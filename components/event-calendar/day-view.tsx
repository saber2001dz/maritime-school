...
export function DayView({
  ...
  // Filter all-day events (only explicitly marked as all-day)
  ...
  // Get time-based events (including multi-day events with specific times)
  const timeEvents = useMemo(() => {
    return dayEvents.filter((event) => {
      // Exclude only explicitly marked all-day events
      return event.allDay !== true
    })
  }, [dayEvents])
  ...
  const positionedEvents = useMemo(() => {
    ...
    sortedEvents.forEach((event) => {
      ...
      // Find the maximum column index among events that overlap with this event's time range
      let maxOverlappingColumnIndex = columnIndex
      for (let i = 0; i < columns.length; i++) {
        ...
        if (hasOverlap) {
          maxOverlappingColumnIndex = Math.max(maxOverlappingColumnIndex, i)
        }
      }

      // The number of columns is the max column index + 1
      const maxOverlappingColumns = maxOverlappingColumnIndex + 1
